...
#include <catch2/catch_all.hpp> //testing lib
#include <iostream> //printing
#include <format> //for awkward print statements
#include <locale> //wide character support
#include <codecvt> //codec conversion

#include "tools.hpp" //tools to be tested
...
namespace dsh
{
    ...
    TEST_CASE("[TEST] generic_pointer construction", "[generic-pointer-constructor]")
    {
        ...
        std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> convert; //because microsoft can't implement anything past utf-8, so we all have to convert to utf-8, wow!
        std::wcout.imbue(std::locale("")); //Windows Data compatibility code
        ...
        std::cout << "TEST {i8_test} value: " << i8_test.get() << " addr: " << i8_test.base() << '\n';
        std::cout << "TEST {i16_test} value: " << i16_test.get() << " addr: " << i16_test.base() << '\n';
        std::cout << "TEST {i32_test} value: " << i32_test.get() << " addr: " << i32_test.base() << '\n';
        std::cout << "TEST {i64_test} value: " << i64_test.get() << " addr: " << i64_test.base() << '\n';

        std::cout << "TEST {ui8_test} value: " << ui8_test.get() << " addr: " << ui8_test.base() << '\n';
        std::cout << "TEST {ui16_test} value: " << ui16_test.get() << " addr: " << ui16_test.base() << '\n';
        std::cout << "TEST {ui32_test} value: " << ui32_test.get() << " addr: " << ui32_test.base() << '\n';
        std::cout << "TEST {ui64_test} value: " << ui64_test.get() << " addr: " << ui64_test.base() << '\n';

        std::cout << "TEST {i8_c_test} value: " << i8_c_test.get() << " addr: " << i8_c_test.base() << '\n';
        std::cout << "TEST {i16_c_test} value: " << i16_c_test.get() << " addr: " << i16_c_test.base() << '\n';
        std::cout << "TEST {i32_c_test} value: " << i32_c_test.get() << " addr: " << i32_c_test.base() << '\n';
        std::cout << "TEST {i64_c_test} value: " << i64_c_test.get() << " addr: " << i64_c_test.base() << '\n';

        std::cout << "TEST {ui8_c_test} value: " << ui8_c_test.get() << " addr: " << ui8_c_test.base() << '\n';
        std::cout << "TEST {ui16_c_test} value: " << ui16_c_test.get() << " addr: " << ui16_c_test.base() << '\n';
        std::cout << "TEST {ui32_c_test} value: " << ui32_c_test.get() << " addr: " << ui32_c_test.base() << '\n';
        std::cout << "TEST {ui64_c_test} value: " << ui64_c_test.get() << " addr: " << ui64_c_test.base() << '\n';

        std::cout << "TEST {f32_test} value: " << f32_test.get() << " addr: " << f32_test.base() << '\n';
        std::cout << "TEST {f64_test} value: " << f64_test.get() << " addr: " << f64_test.base() << '\n';
        std::cout << "TEST {f128_test} value: " << std::format("{:.32g}", f128_test.get()) << " addr: " << std::format("{:p}", static_cast<void *>(f128_test.base())) << '\n';

        std::cout << "TEST {f32_c_test} value: " << f32_c_test.get() << " addr: " << f32_c_test.base() << '\n';
        std::cout << "TEST {f64_c_test} value: " << f64_c_test.get() << " addr: " << f64_c_test.base() << '\n';
        std::cout << "TEST {f128_c_test} value: " << std::format("{:.32g}", f128_c_test.get()) << " addr: " << std::format("{:p}", static_cast<const void *>(f128_c_test.base())) << '\n';

        std::cout << "TEST {c8_test} value: " << c8_test.get() << " addr: " << c8_test.base() << '\n';
        std::cout << "TEST {uc8_test} value: " << uc8_test.get() << " addr: " << uc8_test.base() << '\n';

        std::wstring wstr = convert.from_bytes(c16_test.get());
        std::wstring* wstrptr = &wstr;
        std::wcout << L"TEST {c16_test} value: " << wstr << L" addr: " << wstrptr << '\n';
    ...
    TEST_CASE("[TEST] generic_pointer get() return.", "[generic-pointer::get()-return]")
    {
        ...
        REQUIRE((Same<i8::value_type, decltype(i8_test.get())> && int(i8_test.get()) == 0));
        ...
        REQUIRE((Same<ui8::value_type, decltype(ui8_test.get())> && (unsigned int)ui8_test.get() == 4));
        ...
        REQUIRE((Same<i8_c::value_type, decltype(i8_c_test.get())> && (const int)i8_c_test.get() == 8));
        ...
        REQUIRE((Same<ui8_c::value_type, decltype(ui8_c_test.get())> && (unsigned const int)ui8_c_test.get() == 12));
